...
def convert_to_delta_actions(demo, actions):
    ...
    for t in range(len(actions)):
        ...
        # Parse action components from timestep
...
def process_demo_file(input_path, in_place=True, output_path=None):
    """
    ...
    with h5py.File(input_path, 'r') as f_check:
        ...
        try:
            ...
            print(f"Computed {len(delta_actions)} delta actions from {len(actions)} absolute actions")
...
def process_folder(demos_folder, recursive=False, in_place=True):
    """
    ...
    for input_file in tqdm(hdf5_files, desc="Processing demos"):
        try:
            process_demo_file(input_file, in_place=in_place)
        except Exception as e:
...
def main():
    ...
    if args.single_file:
        ...
        if args.dry_run:
            ...
            process_demo_file(args.single_file, in_place=True)
    else:
        # Process folder
        ...
        if args.dry_run:
            ...
            process_folder(args.demos_folder, recursive=args.recursive, in_place=True)
