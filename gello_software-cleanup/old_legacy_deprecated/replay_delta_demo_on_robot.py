...
def initialize_robot_env(use_cameras=True, control_rate_hz=20.0):
    ...
    print("USING CONTROL RATE: ", control_rate_hz)
    env = RobotEnv(
        robot_client,
    ...
    try:
        for i in range(start_idx, end_idx):
            ...
            if use_dataset_pose:
                ...
                # print(f"Current pose (from DATASET): {curr_pos}")
            else:
                # Use real robot pose from observations
                curr_pose = pose2mat(obs["eef_pos"], obs["eef_quat"])
                print(f"Current pose (from ROBOT): {curr_pose[:3, 3]}")

                curr_pos_data = dataset_eef_pos[i]
                curr_quat_data = dataset_eef_quat[i]
                curr_pose_data = pose2mat(curr_pos_data, curr_quat_data)
                print(f"Current pose (from DATA): {curr_pose_data[:3, 3]}")
