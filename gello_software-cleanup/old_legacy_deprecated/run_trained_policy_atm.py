        ...
        while not done and (horizon is None or step_i < horizon):
            ...
            if len(horizon_actions) == 0:
                ...
                for delta_action in delta_actions:
                    ...
                    converted_actions.append(abs_action)

                horizon_actions = converted_actions
                print("new horizon", horizon_actions)
                # exit()

            for i in range(len(horizon_actions)):
                action = horizon_actions[i]
                print(f"executing {i} action in horizon")

                if not confirm_first_action:
                    input(f"intended action (with quat) [enter to rollout]: {action}")
                    confirm_first_action = True


                obs, act = env.step(action.tolist())
            ...
            save_data = {
                ...
                "output" : {
                    ...
                    "predicted_absolute_actions": horizon_actions
                }
            }
