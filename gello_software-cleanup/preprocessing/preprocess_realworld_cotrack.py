...
def get_task_bert_embs(libero_root_dir):
    ...
    if not os.path.exists("libero/task_embedding_caches/task_emb_bert_pickup_block.npy"):
        ...
        np.save("libero/task_embedding_caches/task_emb_bert_pickup_block.npy", task_name_to_emb)
    else:
        task_name_to_emb = np.load("libero/task_embedding_caches/task_emb_bert_pickup_block.npy", allow_pickle=True).item()
...
def track_through_video(video, track_model, num_points=1000, max_size=128):
    T, C, H, W = video.shape

    print(f"tracking with {num_points} points")

    video = torch.from_numpy(video).cuda().float()

    print("loaded video into tensor")
...
def collect_states_from_demo(h5_file, image_save_dir, demos_group, demo_k, view_names, track_model, task_emb, num_points, visualizer, save_vis=False, max_size=128):
    ...
    for view in view_names:
        rgb = np.array(demos_group[demo_k]['obs'][f'{view}_image'])
        ...
        print(rgb.shape)
        # resize to max_size, use numpy
        rgb = np.stack([
            cv2.resize(frame.transpose(1, 2, 0), (128, 128), interpolation=cv2.INTER_LINEAR).transpose(2, 0, 1)
            for frame in rgb
        ])

        print(rgb.shape)

        pred_tracks, pred_vis = track_through_video(rgb, track_model, num_points=num_points, max_size=max_size)
        ...
        if save_vis:
            ...
            print("saved video to visualizer")
        ...
        if "video" not in view_grp:
            ...
            view_grp.create_dataset("video", data=rgb[None])

            print("saved video to dataset")
...
def generate_data(source_h5_path, target_dir, track_model, task_emb, skip_exist):
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(demo_keys))):
            ...
            try:
                collect_states_from_demo(h5_file_handle, image_save_dir, demos, demo_k, views, track_model, task_emb, num_points, visualizer, save_vis=(idx%10==0), max_size=128)
...
@click.option("--save", type=str, default="./data/pickup_block_atm_cotrack_128/")
@click.option("--suite", type=str, default="pickup_block")
