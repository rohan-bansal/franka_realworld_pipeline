...
#         img_concat = np.concatenate((data['left_image'], data['right_image']), axis=1)
#         depth_concat = np.concatenate((data['left_depth'], data['right_depth']), axis=1)
...
#     debug_read(cam_list[0])
#     # debug_read_all(cam_list)
...
def debug_read(camera):
    ...
    while True:
        ...
        # Display images
        cv2.imshow("ZED Image", img_left)
        cv2.imshow("ZED Depth", depth_left_3ch)
