...
class DataCollector():
    ...
    def _flush_buffer_to_disk(self, buffer):
        ...
        for k in obs_buffer[0]:
            ...
            # try:
            #     stacked = [obs_buffer[i][k] for i in range(len(obs_buffer))]
            #     print(f"[DEBUG] Key: {k}")
            #     print(f"  Sample shape: {stacked[0].shape}")
            #     print(f"  Sample dtype: {stacked[0].dtype}")
            #     print(f"  All same shape: {all(s.shape == stacked[0].shape for s in stacked)}")
            #     obs_chunk_to_flush = np.stack(stacked, 0)
            #     print(f"  Stacked shape: {obs_chunk_to_flush.shape}, dtype: {obs_chunk_to_flush.dtype}")
            #     self.ep_data_grp.create_dataset(f"chunk_{self.chunk_count}/obs/{k}", data=obs_chunk_to_flush)
            # except Exception as e:
            #     print(f"[ERROR] Could not flush obs key '{k}': {e}")
            ...
            if self._buffer_to_flush is not None:
                with self.flush_lock:
                    try:
                        ...
                        print(f"[AsyncFlusher] Exception during flush:\n{traceback.format_exc()}")
