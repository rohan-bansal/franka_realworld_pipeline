...
def generate_dynamic_traces(t, bag_data):
    ...
    print(bag_data[t].keys())
    
    initial_pose = bag_data[t]['initial_pose']
    target_poses = bag_data[t]['target_poses']
    actual_poses = bag_data[t]['actual_poses']

    target_poses.insert(0, initial_pose)
    actual_poses.insert(0, initial_pose)
    ...
    if SHOW_TARGET_POSES:
        dynamic_traces.extend(_create_trajectory_traces(
            target_poses, CUSTOM_GRADIENT_TARGET, 'Target Poses', line_style='dash'
        ))
    
    # Actual trajectory
    if SHOW_ACTUAL_TRAJECTORY:
        dynamic_traces.extend(_create_trajectory_traces(
            actual_poses, CUSTOM_GRADIENT_ACTUAL, 'Executed Poses'
        ))
