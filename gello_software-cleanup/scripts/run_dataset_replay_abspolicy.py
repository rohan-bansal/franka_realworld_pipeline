...
def rollout(env, policy, horizon=None):
    ...
    try:
        while not done and (horizon is None or step_i < horizon):
            ...
            if len(loop_start_times) >= 2 and step_i % frequency_print_interval == 0:
                ...
                if len(recent_times) >= 2:
                    ...
                    print(f"\n--- CHUNK EXECUTION FREQUENCY ---")
                    print(f"Step {step_i}: Actual chunk frequency: {actual_frequency:.2f} Hz")
                    print(f"(This measures how often action chunks are processed, not individual actions)")
                    print(f"----------------------------------\n")
            ...
            for i in range(len(action_queue)):
                ...
                action_execution_times.append(action_duration)
                ...
                print(f"  Action execution time: {action_duration*1000:.2f} ms ({1.0/action_duration:.2f} Hz)")
            ...
            # Print action execution statistics for this chunk
            action_loop_end_time = time.time()
            total_action_time = action_loop_end_time - action_loop_start_time
            if action_execution_times:
                avg_action_time = np.mean(action_execution_times)
                min_action_time = np.min(action_execution_times)
                max_action_time = np.max(action_execution_times)
                avg_action_rate = 1.0 / avg_action_time
                actual_chunk_rate = len(action_execution_times) / total_action_time
                
                print(f"\n--- ACTION EXECUTION STATS (this chunk) ---")
                print(f"Total actions executed: {len(action_execution_times)}")
                print(f"Total time for chunk: {total_action_time*1000:.2f} ms")
                print(f"Average action time: {avg_action_time*1000:.2f} ms ({avg_action_rate:.2f} Hz)")
                print(f"Min action time: {min_action_time*1000:.2f} ms ({1.0/min_action_time:.2f} Hz)")
                print(f"Max action time: {max_action_time*1000:.2f} ms ({1.0/max_action_time:.2f} Hz)")
                print(f"Actual chunk rate: {actual_chunk_rate:.2f} actions/sec")
                # # print(f"Target rate: {env._control_rate_hz} Hz")
                # print(f"Action rate ratio: {avg_action_rate/env._control_rate_hz:.2f}")
                print(f"-------------------------------------------\n")
        ...
        if len(loop_start_times) >= 2:
            ...
            print(f"\n--- INTERRUPTED - CHUNK FREQUENCY STATISTICS ---")
            print(f"Total chunk iterations: {total_iterations}")
            print(f"Total time: {total_time:.2f} seconds")
            print(f"Average chunk frequency: {average_frequency:.2f} Hz")
            print(f"(This measures chunks, not individual actions)")
            print(f"--------------------------------------------------\n")
    ...
    if len(loop_start_times) >= 2:
        ...
        total_iterations = len(loop_start_times) - 1
        average_frequency = total_iterations / total_time
        print(f"\n--- FINAL CHUNK FREQUENCY STATISTICS ---")
        print(f"Total chunk iterations: {total_iterations}")
        print(f"Total time: {total_time:.2f} seconds")
        print(f"Average chunk frequency: {average_frequency:.2f} Hz")
        print(f"(This measures chunks, not individual actions)")
