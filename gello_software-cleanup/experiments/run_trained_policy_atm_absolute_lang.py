...
@torch.no_grad()
def rollout(env, policy, horizon=None, action_norms=None, override_task_emb=None):
    ...
    try:
        while not done and (horizon is None or step_i < horizon):
            ...
            if override_task_emb is not None:
                task_emb = override_task_emb[np.newaxis, ...]
            else:
...
def evaluate(fabric, cfg):
    ...
    # Prompt for language instruction
    instruction = input("Enter language instruction: ")
    if instruction:
        task_emb = get_task_embs([instruction])[0]
        print(f"Computed task embedding for '{instruction}' with shape {task_emb.shape}")
    else:
        task_emb = None
        print("No instruction provided, will use task_emb from obs if available.")

    result = rollout(env, model, horizon=rollout_horizon, action_norms=action_norms, override_task_emb=task_emb)
