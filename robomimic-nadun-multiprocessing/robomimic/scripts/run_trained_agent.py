...
def sample_initial_state_from_npy(sim_state_path, state_dict):
    """
    Read the specified initial state from the path and sample a random initial state.
    Then return a state_dict for initializing the environment before Rollout.
    """
    sim_state_data = np.load(sim_state_path)
    sim_state_len = sim_state_data.shape[0]

    # sample a random initial state
    idx = np.random.randint(0, sim_state_len)
    sim_state = sim_state_data[idx]
    state_dict["states"] = sim_state
    return state_dict

def rollout(policy, env, horizon, render=False, video_writer=None, video_skip=5,
            return_obs=False, camera_names=None, real=False, rate_measure=None,
            rollout_rate=20, receding_horizon=False, sample_initial_state=False):
    """
    ...
    rate = Rate(rollout_rate, name="rollout_rate", log_warning=True)
    ...
    if real:
        ...
        if sample_initial_state:
            # hack for initial state with sampling the initial state of simulator
            sim_state_path = "/coc/flash7/zhenyang/robomimic-nadun/robomimic/dev/plotting/result/sim_states_data.npy"
            state_dict = sample_initial_state_from_npy(sim_state_path, state_dict)

        obs = env.reset_to(state_dict)
        obs["time"] = np.array([0.0]) # might not be accurate, but should right after observation
    ...
    try:
        ...
        for step_i in range(horizon):
            ...
            act, inference_data = policy(ob=obs, parallel_inference=receding_horizon) # dim=(7,), inferen data = {obs, action_seq}
            ...
            if rate_measure is not None:
                ...
                next_obs["time"] = np.array([time.time() - t_start])
                # print(f"One iteration rate: {1/(time.time() - previous_step):.2f}, Time for env step: {time.time() - before_step:.4f}")
            ...
            if inference_data is not None:
                ...
                print("warning: inference data log is None")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--agent",
        type=str,
        default='/home/mbronars/rohan/trained_models/models/model_epoch_2000.pth',
        # required=True,
    ...
    parser.add_argument(
        ...
        action='store_true',
        help="sample initial state from the skill motion",
    )
