...
def train(config, device, auto_remove_exp=False):
    ...
    # Create new model, env_meta and shape_meta
    # load basic metadata from training file
    print("\n============= Loaded Environment Metadata =============")
    env_meta = FileUtils.get_env_metadata_from_dataset(dataset_path=dataset_path, ds_format=ds_format)
    
    # # TODO: fix this, only for the speed tacking short data set.
    # env_meta =  {"env_name": "RL2RobotEnv", "type": 4, "lang": "33", "env_kwargs":
    ...
    if config.train.load_ckpt is None:
        model = algo_factory(
        ...
        config_ckpt, shape_meta_ckpt, env_meta_ckpt, model = TrainUtils.load_dict_from_checkpoint(config.train.load_ckpt, device)
        epoch_start = config.train.epoch_start
        # print(f"************ Loaded checkpoint from {config.train.load_ckpt} ************")
        # print(f"************ Loaded checkpoint from {config.train.load_ckpt} ************")
    ...
    for epoch in range(epoch_start, config.train.num_epochs + 1): # epoch numbers start at 1
        ...
        TrainUtils.save_model(
            ...
            ckpt_path=os.path.join(ckpt_dir, latest_ckpt_name + ".pth"),
            obs_normalization_stats=obs_normalization_stats,
            action_normalization_stats=action_normalization_stats,
        )
...
def main(args):
    ...
    print(f"************ Try to use cuda {config.train.cuda} Device You are Using is: {device}****************")
