...
def rollout_diffusion_policy(policy, env, horizon, render=False, video_writer=None, video_skip=5, return_obs=False,
    ...
    try:
        while actions_remaining > 0:
            ...
            start = time.time()
            act = policy(ob=obs, **kwargs)[0] # Act is a flattened vector of spline coefficients
            ...
            if kwargs['osc_control']:
                ...
                # Generate absolute joint position targets from spline
                per_act_dim_coeefs = act.reshape(KNOTS, NUM_COEFFS, ACT_DIM, order='C')
                knots = [x for x in range(0, SPLINE_LENGTH, SEGMENT_LENGTH)]
                knots.append(SPLINE_LENGTH - 1)
                spline_act = [] #actions generated from the spline
                for d in range(ACT_DIM):
                    coeef = per_act_dim_coeefs[d]
                    spline = PPoly(coeef, knots)  # init the spline
                    sample = spline(SPLINE_SAMPLING_X)
                    spline_act.append(sample)
                spline_act = np.array(spline_act).transpose()

                # Now we step through each spline generated action
                for s in range(spline_act.shape[0]):
                    act = spline_act[s]
    ...
    stats = dict(Return=total_reward, Horizon=num_actual_actions, Success_Rate=float(success),
                 Time_Taken_in_rollout=time.time() - start_rollout)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--agent",
        type=str,
        default="/media/nadun/Data/phd_project/robomimic/bc_trained_models/diffusion_policy/sim/spline/joint_position/can_all_obs_cubic_spline_32/20241115183324/models/model_epoch_1000.pth",
        required=False,
    ...
    if args.video_path is None:
        args.video_path = os.path.abspath(
            os.path.join(os.path.dirname(args.agent), '..', 'videos', f"eval_spline_{datetime.datetime.now()}.mp4"))
    ...
    if args.dataset_path is None:
        args.dataset_path = os.path.abspath(os.path.join(os.path.dirname(args.agent), '..', 'logs',
                                                         f'spline_rollout_{datetime.datetime.now()}.pkl'))
