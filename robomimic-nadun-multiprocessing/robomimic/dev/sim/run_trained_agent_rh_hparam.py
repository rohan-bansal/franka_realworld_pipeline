...
def rollout_diffusion_policy(policy, env, horizon, render=False, video_writer=None, video_skip=5, return_obs=False,
    ...
    _, sim_time_elapsed = env.getSimTimeInfo()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--agent",
        type=str,
        # default="/home/wjung85/Repo/projects/FastIL/diffusion_trained_models/square_image_only_speed_adaptive_model.pth",
        default="/home/wjung85/Repo/projects/FastIL/diffusion_trained_models/cfg_square/square_image_action_horizon_16_additional_bl/20250121005959/models/model_epoch_400.pth",
        # default="/home/wjung85/Repo/projects/FastIL/diffusion_trained_models/model_epoch_1000_can.pth",
        required=False,
    ...
    parser.add_argument(
        ...
        type=str,
        # default=None,
        default="/nethome/nkra3/flash7/phd_project/robomimic-nadun/datasets/can/ph/can_init_states.pkl",
    ...
    parser.add_argument(
        '--config',
        type=str,
        default="/nethome/nkra3/flash7/phd_project/robomimic-nadun/robomimic/dev/hparam/config/can_inpaint/base_config_can.json",
    ...
    if args.video_dir is None:
        if args.debug:
            args.video_dir = os.path.abspath(os.path.join(os.path.dirname(args.agent), '..', 'videos/hparam_debug_sparc'))
        else:
    ...
    for iter in range(args.N_eval):
        if args.video_path is None:
            args.video_path = os.path.abspath(
                os.path.join(args.video_dir, f"{exp_name}_iter_{iter}_{datetime.datetime.now()}.mp4"))

        if args.dataset_path is None:
            if args.debug:
                dataset_dir = os.path.abspath(os.path.join(os.path.dirname(args.agent), '..', 'logs/hparam_debug_sparc'))
            else:
                dataset_dir = os.path.abspath(os.path.join(os.path.dirname(args.agent), '..', 'logs/hparam'))
            args.dataset_path = os.path.join(dataset_dir, f"{exp_name}_iter_{iter}_{datetime.datetime.now()}.pkl")
