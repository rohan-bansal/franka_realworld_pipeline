...
from robosuite.utils.input_utils import input2action, input2teleop_info

import robosuite.environments.libero.bddl_utils as BDDLUtils
from robosuite.environments.libero import *
...
def collect_human_trajectory(
    ...
    while True:
        ...
        # Get the newest teleop_info
        teleop_info = input2teleop_info(
            device=device,
            robot=active_robot,
            active_arm=arm,
        ...
        # If resetting, break
        if teleop_info['reset'] :
            print("Break")
            saving = teleop_info['save_demo']
            break

        # Run environment step
        action = teleop_info['action']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bddl-file",
                        type=str,
                        default="bddl_files/pick_place_all_objects.bddl")
    ...
    if args.device == "keyboard":
        ...
        from robosuite.devices import Quest
        device = Quest(robots=env.robots)
        time.sleep(2)
    else:
        raise Exception(
            "Invalid device choice: choose one in ['keyboard', 'spacemouse', 'quest']."
        )
