...
def rollout(
    ...
    # TODO: MOVE THIS
    kwargs = {"return_action_sequence": False,  "control_mode": "Joint_Position"}
    if return_obs:
...
def rollout_with_action_sequence(policy, env, horizon, render=False, video_writer=None, video_skip=5,
        return_obs=True, camera_names=None, real=False,
        rollout_demo=False, rollout_demo_obs=False, demo=None, demo_act_key="absolute_axis_angle_actions"):
    """
    Helper function to carry out rollouts. Supports on-screen rendering, off-screen rendering to a video,
    and returns the rollout trajectory.
    ...
    try:
        while step_i < horizon:
            ...
            # update for next iter
            if kwargs["step_action_sequence"]:
            ...
            if not kwargs['step_action_sequence']:
                for i in range(run_actions):
                    a = act[i]
                    # TODO: this depends on diffusion policy observation horizon, fix this
                    ...
                    next_obs, r, done, _ = env.step(a, **kwargs)
                    time.sleep(env.robot_interface.j_point_time - 0.01)
            else: # send the whole action sequence using the joint trajectory
                last_time_sent_actions = time.time()
...
def rollout_demo_with_action_sequence(policy, env, horizon, render=False, video_writer=None, video_skip=5,
        return_obs=False, camera_names=None, real=False,
        rollout_demo=False, rollout_demo_obs=False, demo=None, demo_act_key="absolute_axis_angle_actions"):
    """
    Helper function to carry out rollouts. Supports on-screen rendering, off-screen rendering to a video,
    ...
    kwargs = {"return_action_sequence": True, "step_action_sequence": True,
              "control_mode": "Joint_Position_Trajectory", "delta_model": False,
              "temporal_ensemble": True, "spline": True, "inpaint_first_action": False,
              "diffusion_sample_n": 1, "return_all_pred": False, "run_actions" : 16}
    ...
    try:
        while step_i < horizon:
            ...
            if rollout_demo_obs:
                # TODO, add OBSERVATION HORIZON HERE
                obs = demo_obs_to_obs_dict(demo['obs'], step_i)
                obs = env.get_observation(obs)
            ...
            start_inf = time.time()
            inf_start_times.append(start_inf)
            kwargs["inf_start_time"] = env.robot_interface.node.get_clock().now().to_msg()
            ...
            if not kwargs['step_action_sequence']:
                for i in range(run_actions):
                    ...
                    next_obs, r, done, _ = env.step(a, **kwargs)
                    time.sleep(env.robot_interface.j_point_time - 0.01)
                time.sleep(1) # TODO large sleep at end of actions, take this out
            else: # send the whole action sequence using the joint trajectory
...
if __name__ == "__main__":
    ...
    args.agent = "/home/robot-aiml/ac_learning_repos/robomimic-nadun/bc_trained_models/real_robot/serve_apple/absolute_joint_control_all_obs_seq_32/20241226162950/models/model_epoch_600.pth"
    args.agent = "/home/robot-aiml/ac_learning_repos/robomimic-nadun/bc_trained_models/real_robot/serve_apple/absolute_joint_control_all_obs_seq_32/20241227173142/models/model_epoch_1000.pth"

    # args.dataset_path = "/home/robot-aiml/ac_learning_repos/experiment_logs/pick_cube_11_7/absolute_joint_control_no_framestack_all_obs_seq_32_run_10_1x_all_fixes.pkl"
