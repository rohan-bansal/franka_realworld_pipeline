...
# TODO: do we take out this verbose flag?
def initialize_obs_utils_with_obs_specs(obs_modality_specs, verbose=False):
    """
...
def resize_image(im, output_shape, image_type=None):
    """
    @param images: image of shape (h,w,c) or batch of images of shape (b,h,w,c)
    @param output_shape: tuple of (h',w') to resize to
    @return: resized batch of images after center cropping
    """
    if image_type == 'rgb' or im.shape[-1] == 3:
        im = np.squeeze(im)
        assert im.shape[-1] == 3, "RGB image must have 3 channels!"
        interpolation = Image.BILINEAR
    if image_type == 'depth' or im.shape[-1] == 1:
        assert im.shape[-1] == 1, "Depth image must have 1 channel!"
        interpolation = Image.NEAREST

    if im.ndim > 3: # batch of images
        im_list = []
        for i in range(im.shape[0]):
            crop_size = min(im[i].shape[0], im[i].shape[1])
            cropped_image = center_crop(im[i], crop_size, crop_size)
            print(f"cropped image size {cropped_image.shape} im size {im.shape}")
            resized_image = Image.fromarray(cropped_image).resize(output_shape, interpolation)
            im_list.append(resized_image)

        images = np.stack(im_list, axis=0)
        return images

    else: # single image
        crop_size = min(im.shape[0], im.shape[1])
        cropped_image = center_crop(im, crop_size, crop_size)
        resized_image = Image.fromarray(cropped_image).resize(output_shape, interpolation)
...
def normalize_dict(dict, normalization_stats):
    ...
    for m in dict:
        ...
        # if np.any(np.abs(dict[m]>1.0)):
        #     print(f"Normalization fail for {m}")
        #     assert False
