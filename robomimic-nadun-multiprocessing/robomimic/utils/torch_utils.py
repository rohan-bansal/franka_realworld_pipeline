...
"""
def positions_to_linvels(positions: torch.Tensor, dt: float):
    """
    Numerical differentiation of position to get velocity

    Args:
        positions: (T, 3) tensor of positions
        dt: time step

    Returns:
        (T-1, 3) tensor of linear velocities
    """
    return (positions[..., 1:, :] - positions[..., :-1, :]) / dt

# @time_profile
def axis_angle_to_angvel(axis_angles: torch.Tensor, dt: float, max_omega: float=4.0):
    """
    Numerical differentiation of axis angle to get angular velocity (vectorized version)

    Args:
        axis_angles: (T, 3) tensor of axis angles
        dt: time step
        max_omega: maximum angular velocity

    Returns:
        (T-1, 3) tensor of angular velocities
    """
    num_points = axis_angles.shape[-2]
    coordinate = axis_angles.shape[-1]
    ...
    rotation_mats = axis_angle_to_matrix(axis_angles)
    
    # Calculate dR for all timesteps at once
    R_i = rotation_mats[..., :-1, :, :]  # (B, T-1, 3, 3)
    R_ip1 = rotation_mats[..., 1:, :, :]  # (B, T-1, 3, 3)
    
    dR = (R_ip1 - R_i) / dt  # (B, T-1, 3, 3)
    omega_mat = dR @ R_i.transpose(-2, -1)  # (B, T-1, 3, 3)
    
    # Extract angular velocities from skew-symmetric matrix
    omega = torch.stack([
        omega_mat[..., 2, 1],  # ω_x
        omega_mat[..., 0, 2],  # ω_y
        omega_mat[..., 1, 0]   # ω_z
    ...
    # Clamp angular velocities if needed
    if torch.any(torch.abs(omega) > max_omega):
        # if torch.is_grad_enabled():
        #     print(f"Warning: Omega is larger than {max_omega}")
        omega = torch.clamp(omega, -max_omega, max_omega)
    
    return omega

def pose_to_twist(poses: torch.Tensor, dt: float):
    """
    Numerical differentiation of pose to get twist

    Args:
        poses: (T, 6) tensor of poses [x, y, z, ax, ay, az]
        dt: time step

    Returns:
        (T-1, 6) tensor of twists
    """
    eef_pos = poses[..., :3]
    eef_axis_angles = poses[..., 3:6]
    eef_pos_vel = positions_to_linvels(eef_pos, dt)
    eef_ang_vel = axis_angle_to_angvel(eef_axis_angles, dt)
    eef_twist = torch.cat([eef_pos_vel, eef_ang_vel], dim=-1)
...
if __name__ == "__main__":
    ...
    pkl_files = pkl_utils.get_result_pkl_files(result_dir, filter="bl")

    for demo_idx in range(50):
        demo = pkl_utils.load_demo_from_pkl(pkl_files[0], demo_idx=demo_idx)

        demo_poses = torch.tensor(np.array(demo["executed_actions"]))
        twist_truth = torch.tensor(demo_utils.get_end_effector_twist(demo, type="cmd"))
        twist_test  = pose_to_twist(demo_poses, demo_utils.get_dt(demo))

        error = torch.norm(twist_truth - twist_test.cpu(), dim=-1).mean()
        
        if error > 1e-3:
            print(f"demo {demo_idx} diff: {error}")