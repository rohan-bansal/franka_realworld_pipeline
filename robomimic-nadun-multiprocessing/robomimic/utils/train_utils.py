...
def get_exp_dir(config, auto_remove_exp_dir=False):
    ...
    if os.path.exists(base_output_dir) and config.train.load_ckpt is None:
...
def load_dict_from_checkpoint(ckpt_path, device):
    ...
    Return config, shape_meta, env_meta and constructed model
    """
    # Load model parameters
    params = torch.load(ckpt_path) #, weights_only=True)
    
    # Load config
    saved_cfg = json.loads(params["config"]) # load from saved json string
    config = config_factory(params["algo_name"])
    ...
    # Load metadata
    shape_meta = params["shape_metadata"]
    env_meta = params["env_metadata"]
    
    # Create model with saved config, to ensure that the model setup is correct wrt weights
    model = algo_factory(
    ...
    return config, shape_meta, env_meta, model
